...
The next bug is a bit trickier: The search result is returned in chunks from the Extractor, but in the way they were created, items being in previous chunks could reappear in following ones, as [@PhoenixDiscord](https://github.com/PhoenixDiscord) noticed. Not anymore, thanks to [@gzsombor](https://github.com/gzsombor). [@7FM](https://github.com/7FM) fixed an Extractor crash which happened sometimes when videos got played.
