...
The player now uses the original audio language by default again, instead of preferring autotranslated audio tracks. We are sorry for initially choosing "Yes" as the default value for the "Prefer original audio" option. If needed, you can switch it back by going to settings.
...
A bug was fixed that caused insufficient permissions to open downloads directly from within the app. Also, after hearing a suggestion from some user we met at [FrOSCon](https://newpipe.net/blog/talk/pinned/froscon-2025/), we added a button to remove a download entry in NewPipe without deleting the actual file. This was actually coded on the spot during the conference :-D
