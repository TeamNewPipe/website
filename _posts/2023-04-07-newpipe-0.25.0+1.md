---
layout: post
title: "NewPipe 0.25.0 and 25.1 released: Card view, tapable URLS/timestamps in comments, and PeerTube search filters ahoy!"
...
In the meantime, another release came out. This is mostly a bugfix release, so we decided to include the contents of that as well. Sorry for the resulting length, we put more screenshots in then usually to make it easier to scroll through. 
...
- [@Jared234](https://github.com/Jared234) improved the "Add to playlist" dialog tremendously regarding duplicates. While they didn't get a special treatement before, now playlist already containing the stream will get greyed out. If you still decide to add the stream to the playlist, you also get a small toast telling you what you have done.
...
<figure class="center">
    ...
    <figcaption class="text-center">It is good music.</figcaption>
</figure>

- In another entry in their fight against duplicates, [@Jared234](https://github.com/Jared234) added an option to remove duplicates from a playlist.

- [@mahendranv](https://github.com/mahendranv) in their second mention in this post improved the channel card in search results: bigger icon, more lines of the description and moved the video amount. Looks way better now!

- [@NyanCatTW1](https://github.com/NyanCatTW1) with the help of [@Stypox](https://github.com/Stypox) added an option to the `Video and audio` category, disabling hardware media button events.

- [@Jared234](https://github.com/Jared234) finally improved the "What's New" feed, merging all options into one icon and added ignoring partially watched videos to the new menu.

<figure class="center">
    <img src="{{ site.baseurl }}/img/screenshots/whats_new_options.png" class="img-responsive">
    <figcaption class="text-center">Fits perfectly to "What's New" now being a tab by default.</figcaption>
</figure>
...
- [@Stypox](https://github.com/Stypox) decreased a bit the size of thubmnails in the card view which were increased in 25.0 based on feedback from the community.
...
- [@Redirion](https://github.com/Redirion) made a small portion of our users happy whose accelerometer is missing but the device has a bug where it still reports auto rotation as working, thus hiding the full screen button. Now it is shown again.

- [@pratyaksh1610](https://github.com/pratyaksh1610) fixed a crash when pressing on empty comments. Still an open question why you would do that, but now you can. Have fun!

- [@Stypox](https://github.com/Stypox) made NewPipe correctly open URLs in browser, and also fixed opening downloads and external players, all in one PR.

- [@pratyaksh1610](https://github.com/pratyaksh1610) in a second contribution fixed the progress bar on the thumbnail in card view.

- [@Jared234](https://github.com/Jared234) fixed a bug where the playlist thumbnail doesn't get updated when the videos thumbnail/order change in the playlist.
...
- [@Stypox](https://github.com/Stypox) reverted the `WindowCompat` change from 25.0 because of emerged issues.

- [@Redirion](https://github.com/Redirion) updated ExoPlayer to 2.18.5.
