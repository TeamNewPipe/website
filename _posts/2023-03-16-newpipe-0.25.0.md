---
layout: post
title: "NewPipe 0.25.0 released: Card view, PeerTube search filters, and comment replies ahoy!"
...
Damnit, we certainly didn't have it in us to get this blog post out in time. Thanks to <a href="https://github.com/opusforlife2">@Opusforlife</a>, it did materialize after all and not vanish in writers hell.
...
- [@FireMasterK](https://github.com/FireMasterK) started work on adding support for extracting audio track information to the Extractor! This is intended for videos which have multiple audio tracks, such as different languages. Eventually, the Extractor will expose this info so that clients can make use of them. Just a heads up for all the people complaining about Arabic Mr Beast.  

- [@FireMasterK](https://github.com/FireMasterK) also changed how YouTube comments are parsed so that the timestamps and URLs are tappable. Moreover, the Extractor now also extracts the total comment count for clients to show.  

- [@TobiGr](https://github.com/TobiGr) added support for comment replies on PeerTube!  

- [@TobiGr](https://github.com/TobiGr) also added support for searching for playlists and channels on PeerTube!  

- [@Isira-Seneviratne](https://github.com/Isira-Seneviratne) made the app show a toast when no updates are available upon checking manually.  

- [@Theta-Dev](https://github.com/Theta-Dev) has added the subscriber count below the channel name on the video details page. Even more details! All the details!  

- [@Jared234](https://github.com/Jared234) added a Download option to the long-press menu of videos in the queue.  

- [@Jared234](https://github.com/Jared234) also made it so that manually setting a local playlist's thumbnail makes it permanent. The playlist long-press menu has been reworked to add a new option to unset the thumbnail. (If you're wondering where to find the option to set a thumbnail in the first place, it's by long-pressing on a playlist video.)  
...
- [@AudricV](https://github.com/AudricV) added support for extracting the duration for video premieres (upcoming videos). Playlist items now also show their upload date and view count.  
...
- [@AudricV](https://github.com/AudricV) added support for the new "live" URLs on YouTube (of the form `https://www.youtube.com/live/LIVE_ID`), so the app doesn't reject these URLs as unsupported anymore. We proud ourselves to be inclusive of anything, including links. 
...
- [@shivambeohar](https://github.com/shivambeohar) removed an empty space to the right of mini-player close button area, thus making it larger  

- [@Isira-Seneviratne](https://github.com/Isira-Seneviratne) improved the thumbnails in the notification bar by using smoother bitmap downscaling, which made the resulting image way better.  

- [@pratyaksh1610](https://github.com/pratyaksh1610) renamed the "Help" button to "Fast mode" in the "what's new" feed menu to make it self explanatory.  
...
- [@Jared234](https://github.com/Jared234) also changed the seeking behaviour so that it no longer pauses the stream being played.  

- [@pratyaksh1610](https://github.com/pratyaksh1610) added icon for the "Play All" button in channels and playlists.  
...
- [@Redirion](https://github.com/Redirion) made the Samsung DeX availability check run only on Samsung devices, because it's pointless to run it on your Super-pHone Ultpro MinMax.  
...
- [@Marius1501](https://github.com/Marius1501) also changed the YouTube Chapters icon in the player from a numbered list (which looked very similar to the queue icon, a bulleted list) to an open book.  

- [@AudricV](https://github.com/AudricV) added support for YouTube livestream links, the ones which have `/live/` as part of the URL.  

### Fixed

- [@Theta-Dev](https://github.com/Theta-Dev) fixed the extraction of subscriber and video count in search results for YouTube channels which have handles.  
...
- [@AudricV](https://github.com/AudricV) fixed the extraction of Likes on SoundCloud.  
...
- [@Douile](https://github.com/Douile) made the `Enqueue next` option show only when in the middle of a queue, not at the end, where it had the same effect as `Enqueue`.  

- [@dngray](https://github.com/dngray) removed the dead Privacy Tools PeerTube instance from the manifest.  
...
- [@Jared234](https://github.com/Jared234) fixed a bug where using the option to `Remove watched videos` from a playlist also removed the currently playing video from the queue. This also fixed a bug with the same underlying cause, where deleting videos from a playlist and then using the option caused the deleted videos to reappear.  
...
- After [@FireMasterK](https://github.com/FireMasterK) improved the underlying code, [@Stypox](https://github.com/Stypox) prevented ellipsised links from being tapped, to not allow garbage URLs to be opened. Previously, too many characters were being truncated while ellipsising, which has now been limited to the last 2 characters.  
...
- dependabot updated: gson, jumit-bom, and jsoup  
...
- [@TacoTheDank](https://github.com/TacoTheDank) updated Sonarqube, ACRA, Android Gradle Plugin, and Desugaring libraries. [@TobiGr](https://github.com/TobiGr) and [@Isira-Seneviratne](https://github.com/Isira-Seneviratne) later updated the latter two.  
...
- [@Isira-Seneviratne](https://github.com/Isira-Seneviratne) refactored `VideoDetailFragment` and `VideoPlayerUi` to use lambdas.  

- [@Isira-Seneviratne](https://github.com/Isira-Seneviratne) updated RxJava and RxAndroid, which should reduce power consumption on mobile devices! You can spend all that power on watching more videos, presumably.  
