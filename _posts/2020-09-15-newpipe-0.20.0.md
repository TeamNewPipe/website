...
Development. Where all the magic happens which keeps this app running. It saw some improvements in this release (Warning: Nerd talk ahead): A checkstyle rule was added so it would error when a local variable which could be `final`wasn't declared as such (by [@XiangRongLin](https://github.com/XiangRongLin)), numbers and letters are allowed in the app package id (by [@Stypox](https://github.com/Stypox)), contributors need to to discuss their contributions now prior to open a PR (by [@gkeegan](https://github.com/gkeegan)) and some depreciations as well as a little code cleanup happened (by [@TacoTheDank](https://github.com/TacoTheDank).