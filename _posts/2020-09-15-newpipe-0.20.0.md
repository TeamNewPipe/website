...
Have you ever shared a URL to NewPipe, just to be greeted with a little "Unsupported URL" toast? Well, the app still doesn't support those kinds of URLs with this release, but at least we show you a way better error message now! It allows you to reshare the URL or just offers you to open it in a browser instead. All this, thanks to [@Stypox](https://github.com/Stypox) and to [@webber-naut](https://github.com/webber-naut) for bringing our attention to this in the first place. 

Talking about sharing, when you share a live stream to someone, NewPipe doesn't add a timestamp to the URL any more. This was done by [@nmurali94](https://github.com/nmurali94), thanks.
...
Apart from the language improvements mentioned above, some other interesting localization changes happend in this release. [@comradekingu](https://github.com/comradekingu), a frequent contributor to and language checker of NewPipe and other Open Source projects, has once again taken a close look at our strings. There was, as always, a lot of room for improvement. Our wonderful translators rushed to help and immediately implemented his suggestions.

We are also happy to provide **new localizations** in this release: Sardinian, Bengali, Javanese and Berber

If you are missing a translation or think a phrase needs adjustmemnt, head over to [Weblate](https://hosted.weblate.org/engage/newpipe/) and help us to improve NewPip's translation even more!
...
An empty error report in certain kinds of crashes is not empty any more, since [@Stypox](https://github.com/Stypox) took care of properly calling code. Yay, code!

Thanks to [@wb9688](https://github.com/wb9688), you can open a video in the local playlist tab, whereas earlier, the app would crash. And thanks to [@BoFFire](https://github.com/BoFFire), if you open the app in the Taqbaylit language, its called `Taqbaylit` now and not `Tamaziɣt Taqbaylit`.
...
[@mhmdanas](https://github.com/mhmdanas) changed a code call because its origin was redundant. Yay less redundancy. And [@wb9688](https://github.com/wb9688) fixed a crash when a video wouldn't have any next videos but NewPipe wanted to add one to the queue since auto-play were enabled.


## Development

Development. Where all the magic happens which keeps this app running. It saw some improvements in this release (Warning: Nerd talk ahead): A Checkstyle rule was added so it would error when a local variable which could be `final` wasn't declared as such (by [@XiangRongLin](https://github.com/XiangRongLin)), numbers and letters are allowed in the application ID (by [@Stypox](https://github.com/Stypox)), contributors need to to discuss their contributions now prior to open a PR (by [@gkeegan](https://github.com/gkeegan)) and some deprecations as well as a little code cleanup happened (by [@TacoTheDank](https://github.com/TacoTheDank)).
