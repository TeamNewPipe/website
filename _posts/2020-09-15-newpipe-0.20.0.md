...
In order to save yourself some of that valuable mobile data, you were already able to turn of thumbnails of videos and commenting channels. Now the actual images of said commenters are gone (instead of just getting replaced with balck versions of themselves) and you can see a bit more of the comment. Thanks [@4D17Y4](https://github.com/4D17Y4)!
...
An empty error report in certain kinds of crashes is not empty anymore, since [@Stypox](https://github.com/Stypox) took care of properly calling code. Yay, code! Stypox also fixed a crash when NewPipe faced empty comments, now it will display it just as empty as it is.
...
- The extractor will ignore OTF streams now, since they don't work with progressive HTTP and the extractor can't work with DASH just yet ([@wb9688](https://github.com/wb9688))

- PrettyTime is updated to 4.0.6, so a workaround can be removed from NewPipe's code and a perfomance improvement is part of it now ([@B0pol](https://github.com/B0pol)
...
- Disallowing the YouTube website to access the available Java packages from NewPipe's parser ([@wb9688](https://github.com/wb9688))
